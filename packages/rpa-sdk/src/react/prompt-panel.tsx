...
export function PromptPanel({
  ...
  const { status, run, apply, lastPlan, lastExecution, error, setError } = useRpa();
  ...
  async function handlePlan(): Promise<void> {
    ...
    try {
      ...
      if (plan.actions.length === 0) {
        setNotice("No actions were generated. Try giving more explicit instructions.");
        return;
      }

      setNotice(`Plan created: ${plan.actions.length} action(s). Review and click Apply.`);
    } catch (planError) {
      setNotice(planError instanceof Error ? planError.message : "Planning failed.");
    }
  }
  ...
  function handleApply(): void {
    ...
    setNotice(`Applied ${report.applied} action(s), skipped ${report.skipped}.`);
  }
    ...
    <section className={wrapperClass}>
      <div className="rounded-2xl border border-slate-700 bg-slate-950/95 p-4 shadow-2xl backdrop-blur">
        <div className="mb-3 flex items-center justify-between">
          <p className="text-xs uppercase tracking-[0.15em] text-cyan-300">RPA Prompt Panel</p>
          <p className="text-[11px] text-slate-400">status: {status}</p>
        </div>

        <div className="space-y-3">
          <label className="block">
            <span className="mb-1 block text-xs text-slate-300">Instruction</span>
            <textarea
              rows={2}
              value={instruction}
        ...
        <div className="mt-4 border-t border-slate-800 pt-3">
          <p className="text-xs font-medium uppercase tracking-[0.12em] text-slate-400">Action Plan</p>
          {!lastPlan ? (
            <p className="mt-2 text-xs text-slate-500">No plan yet.</p>
          ) : lastPlan.actions.length === 0 ? (
            <p className="mt-2 text-xs text-slate-500">Planner returned no actions.</p>
          ) : (
          ...
          {combinedWarnings.length > 0 ? (
            <div className="mt-3 rounded-md border border-amber-400/30 bg-amber-400/10 p-2">
              <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-amber-200">Warnings</p>
