import type { BlobListResult, BlobPutOptions } from "@vercel/blob";
...
export class VercelBlobStorageAdapter implements StorageAdapter {
	...
	async saveWorkspace(
		...
		await put(stateBlobPath, JSON.stringify(serialized), {
			access: this.#access,
			token: this.#token,
		});
