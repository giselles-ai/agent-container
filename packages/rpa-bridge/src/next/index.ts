...
async function readSlug(context: RouteContext): Promise<string[]> {
  const resolved = await Promise.resolve(context.params).catch(() => undefined as { slug?: string[] } | undefined);
...
function createReadmeSafeError(code: string, message: string, status: number): Response {
...
async function createEventsRoute(request: Request, routeSegments: string[]): Promise<Response> {
  if (routeSegments.length !== 1) {
    return notFoundResponse();
  }

  const sessionId = routeSegments[0] === "events" ? (new URL(request.url).searchParams.get("sessionId") ?? "") : "";
  const token = routeSegments[0] === "events" ? (new URL(request.url).searchParams.get("token") ?? "") : "";
...
async function createDispatchRoute(request: Request): Promise<Response> {
  ...
  try {
    ...
    const bridgeError = toBridgeError(error);
    return createReadmeSafeError(bridgeError.code, bridgeError.message, bridgeError.status);
  }
}
...
export function createBridgeRoutes(): {
  ...
  const getRoute = async (request: Request, context: RouteContext): Promise<Response> => {
  ...
  const postRoute = async (request: Request, context: RouteContext): Promise<Response> => {
    const slug = await readSlug(context);
    if (slug.length === 0) {
      return notFoundResponse();
    }
