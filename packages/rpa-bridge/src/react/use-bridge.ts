...
export function useBridge({ endpoint }: UseBridgeOptions): BridgeHookState {
    ...
    async (event: unknown) => {
      ...
      try {
        ...
        if (event.type === "execute_request") {
          ...
          setWarnings((current) => dedupeStringArray([...current, ...report.warnings]));
      ...
      try {
        ...
        for (const candidate of bridgeBaseCandidates) {
          try {
            ...
            if (!response.ok) {
              latestError = new Error(`Failed to create bridge session (${response.status}).`);
              continue;
            }
        ...
        setStatus("error");
        setError(nextError instanceof Error ? nextError.message : "Failed to initialize bridge.");
      }
    };
    ...
    (event: StreamEvent) => {
      ...
      if (event.type === "message") {
        ...
        if (role === "assistant") {
          if (isDelta) {
            ...
            setMessages((current) =>
              current.map((message) => (message.id === currentId ? { ...message, content: merged } : message))
            );
            return;
          }
        ...
        if (role === "user" && content.trim().length > 0) {
          setMessages((current) => {
            ...
            return [...current, { id: crypto.randomUUID(), role: "user", content }];
          });
        }
      ...
      if (event.type === "tool_result") {
        ...
        if (parsedWarnings.length > 0) {
          setWarnings((current) => dedupeStringArray([...current, ...parsedWarnings]));
        }
        return;
      }
  ...
  const sendMessage = useCallback(
    async ({ message, document }: { message: string; document?: string }) => {
      ...
      if (status !== "connected") {
        const nextError = "Bridge is disconnected. Reload the page and reconnect.";
        setError(nextError);
        throw new Error(nextError);
      }
      ...
      const response = await fetch(`${currentBridgeBase}/chat`, {
        ...
        body: JSON.stringify({
          message: document?.trim() ? `${trimmedMessage}\n\nDocument:\n${document.trim()}` : trimmedMessage,
