	...
	return (
		<div className="mx-auto max-w-3xl rounded-2xl border border-slate-700/60 bg-slate-900/50 p-6 shadow-2xl backdrop-blur">
			...
			<p className="mt-3 text-sm text-slate-300/90">
				Use chat to request form filling. The assistant will plan and apply DOM
				...
				try {
					...
					const response = await fetch("/api/rpa", {
						...
						body: JSON.stringify({
							...
							document:
								toolInput.document ?? (documentText.trim() || undefined),
							fields,
						}),
					});
	...
	const renderedMessages = useMemo(
		() =>
			messages.map((message) => (
				<div
					key={message.id}
					className="space-y-2 rounded-lg border border-slate-800 bg-slate-900/80 p-3"
				>
					...
					{message.parts.map((part, index) => {
						...
						if (part.type === "tool-fillForm") {
							...
							return (
								<div
									key={part.toolCallId}
									className="rounded-md border border-slate-700 bg-slate-950/70 p-2 text-xs text-slate-200"
								>
									...
									{part.output.warnings.length > 0 ? (
										<ul className="mt-1 space-y-1 text-amber-200">
											{part.output.warnings.map((warning, warningIndex) => (
												<li key={`${part.toolCallId}-warn-${warningIndex}`}>
													- {warning}
												</li>
											))}
										</ul>
