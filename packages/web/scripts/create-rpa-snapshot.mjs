...
const TIMEOUT_MS = Number.parseInt(process.env.RPA_SNAPSHOT_TIMEOUT_MS || "2700000", 10);
const GEMINI_PACKAGE = process.env.RPA_GEMINI_PACKAGE?.trim() || "@google/gemini-cli";
...
const SKIP_DIR_NAMES = new Set(["node_modules", "dist", ".next", ".git", ".jj"]);
...
async function main() {
  ...
  console.log(`[snapshot] creating sandbox runtime=${RUNTIME} timeoutMs=${TIMEOUT_MS}...`);
  ...
  try {
    ...
    console.log(`RPA_MCP_SERVER_DIST_PATH=${SANDBOX_ROOT}/packages/mcp-server/dist/index.js`);
