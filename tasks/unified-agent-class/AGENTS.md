...
| 1 | [phase-1-provider-integration.md](./phase-1-provider-integration.md) | ✅ DONE | Update `giselle-provider` to accept `Agent`, call `prepare()` in `doStream()` |
| 2 | [phase-2-route-handler.md](./phase-2-route-handler.md) | ✅ DONE | Update `packages/web/app/api/chat/route.ts` to use `Agent.create()` |
